...
#define EAP_PASSWORD           "Pineapple222003#"
...
String Web_App_URL = "https://script.google.com/macros/s/AKfycbyXa19t7T_-5Uh0iCWRCwsO3zAcL_N17D8kqPUMM8CINyGb-RXRqU052v-TLM1vcvjc/exec";
...
const int numSamples = 400;  // Number of samples to store
...
void calculateAverages() {
  ...
  for (int i = 0; i < numSamples-200; i++) {
    sumLow += adcReadings[i];
  }
  avgLowValues = sumLow / 200.0;  // Using 100.0 for float division
  
  // Calculate average of last 100 values (highest)
  for (int i = numSamples - 200; i < numSamples; i++) {
    sumHigh += adcReadings[i];
  }
  avgHighValues = sumHigh / 200.0;
}
...
//removed functions being called
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    String Send_Data_URL = Web_App_URL + "?sts=write" + "&ratio1=" + String(ratio1) + "&ratio2=" + String(ratio2) + "&ave=" + String(ave);

     if ((lat != "0.00") &&(lng != "0.00")) {
      Send_Data_URL = Web_App_URL + "?sts=write" + "&ratio1=" + String(ratio1) + "&ratio2=" + String(ratio2)+ "&lat=" + String(lat)+ "&long=" + String(lng) + "&ave=" + String(ave);   
    } 
